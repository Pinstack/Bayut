...
A comprehensive command-line interface for scraping, database operations, and utilities.
"""
...
def setup_environment():
    ...
    if not os.path.exists('.venv'):
        logger.error("Virtual environment not found. Please run: python -m venv .venv && source .venv/bin/activate")
        sys.exit(1)
...
def db_status_command(args):
    ...
    try:
        ...
        properties_with_agency = session.query(Property).filter(Property.agency_id.isnot(None)).count()
        properties_with_agent = session.query(Property).filter(Property.agent_id.isnot(None)).count()
        ...
        print(f"Properties with Agency: {properties_with_agency:,} ({properties_with_agency/property_count*100:.1f}%)")
        print(f"Properties with Agent: {properties_with_agent:,} ({properties_with_agent/property_count*100:.1f}%)")
        print("="*50)
...
Examples:
  ...
  python bayut.py info
    ...
    scrape_parser.add_argument('--limit', type=int, help='Limit number of properties to scrape')
    scrape_parser.add_argument('--all', action='store_true', help='Scrape all available properties')
    ...
    # db:backfill
    db_backfill_parser = db_subparsers.add_parser('backfill', help='Backfill normalized tables')
    db_backfill_parser.set_defaults(func=db_backfill_command)
    
    # db:normalize-locations
    db_normalize_parser = db_subparsers.add_parser('normalize-locations', help='Normalize location data')
