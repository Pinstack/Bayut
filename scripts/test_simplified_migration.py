    ...
    try:
        ...
        for i, prop in enumerate(sample_properties, 1):
            ...
            if prop.location_id is not None:
                location = session.query(SimplifiedLocation).filter_by(id=prop.location_id).first()
                if location:
                    print(f"  Location Details: {location.name} (Level {location.level})")
                else:
                    print(f"  ⚠️  Location ID {prop.location_id} not found!")
            else:
                print(f"  ℹ️  No location ID (using location string only)")
...
def test_location_queries():
    ...
    try:
        ...
        riyadh_properties = session.query(SimplifiedProperty).filter(
            SimplifiedProperty.location.ilike('%Riyadh%')
        ).count()
        print(f"   Found: {riyadh_properties} properties")
        
        # Test 2: Find properties with location_id
        print("\n2. Properties with location_id:")
        properties_with_location = session.query(SimplifiedProperty).filter(
            SimplifiedProperty.location_id.isnot(None)
        ).count()
        ...
        properties_without_location = session.query(SimplifiedProperty).filter(
            SimplifiedProperty.location_id.is_(None)
        ).count()
        ...
        if sample_location:
            ...
            location_properties = session.query(SimplifiedProperty).filter_by(
...
def test_performance_comparison():
    ...
    try:
        ...
        simplified_result = session.query(SimplifiedProperty).filter(
            SimplifiedProperty.location.ilike('%Riyadh%')
        ).count()
        ...
        start_time = time.time()
        complex_result = session.query(Property).join(
            PropertyLocation, Property.id == PropertyLocation.property_id
        ).join(
            UniqueLocation, PropertyLocation.location_id == UniqueLocation.id
        ).filter(
            UniqueLocation.name.ilike('%Riyadh%')
        ).count()
        complex_time = time.time() - start_time
        
        print(f"New System (Simplified):")
        print(f"  Query: Simple location string search")
        print(f"  Result: {simplified_result} properties")
        print(f"  Time: {simplified_time:.4f} seconds")
        
        print(f"\nOld System (Complex):")
        print(f"  Query: Multi-table join with location hierarchy")
...
def test_data_consistency():
    ...
    try:
        ...
        # Check location data
        locations_with_properties = session.query(SimplifiedLocation).join(
            SimplifiedProperty, SimplifiedLocation.id == SimplifiedProperty.location_id
        ).count()
