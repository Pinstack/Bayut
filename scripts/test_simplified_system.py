...
from src.models import Property, Location, Agency, Agent, Project, Media, PaymentPlan, Document
...
def test_table_structure():
    ...
    with engine.connect() as conn:
        ...
        expected_tables = ['agencies', 'agents', 'documents', 'locations', 'media', 'payment_plans', 'projects', 'properties']
    ...
    try:
        ...
        if sample_property:
            ...
            if sample_property.location_ref:
                logger.info(f"  Canonical Location: {sample_property.location_ref.name}")
        
        # Test location hierarchy
        root_locations = session.query(Location).filter(Location.parent_id.is_(None)).all()
        ...
        for loc in root_locations[:3]:  # Show first 3
            ...
            children = session.query(Location).filter(Location.parent_id == loc.id).all()
...
def test_queries():
    ...
    try:
        ...
        properties_in_riyadh = session.query(Property).filter(
            Property.location.ilike('%riyadh%')
        ).count()
        logger.info(f"Properties in Riyadh: {properties_in_riyadh}")
        
        # Test price range queries
        expensive_properties = session.query(Property).filter(
        ...
        top_agencies = session.query(Agency).join(Property).group_by(Agency.id).order_by(
            func.count(Property.id).desc()
        ).limit(5).all()
        
        logger.info("Top agencies by property count:")
        for agency in top_agencies:
            prop_count = session.query(Property).filter(Property.agency_id == agency.id).count()
