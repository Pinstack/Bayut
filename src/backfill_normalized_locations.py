...
from models import Location, UniqueLocation, PropertyLocation
...
def backfill_normalized_locations():
    ...
    try:
        ...
        logger.info(f"Found {len(unique_locations_dict)} unique locations across {len(location_hierarchy)} levels")
            ...
            for loc_data in location_hierarchy[level]:
                ...
                if level > 0:
                    ...
                    if loc_data['slug']:
                        ...
                        if parent_slug:
                            # Find parent by slug
                            parent_location = session.query(UniqueLocation).filter(
                                UniqueLocation.slug == parent_slug
                            ).first()
                            if parent_location:
                ...
                location = session.query(UniqueLocation).filter_by(
                    external_id=loc_data['external_id'],
                    level=loc_data['level']
                ).first()
                
                if location:
                    location_id_map[(loc_data['external_id'], loc_data['level'])] = location.id
        ...
        session.commit()
        logger.info(f"Created {property_location_count} property-location relationships")
        ...
        logger.info(f"  Deduplication ratio: {original_location_count / unique_location_count:.1f}:1")
