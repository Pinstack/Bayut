...
from models import Property, Base, Location, Media, Agency, Agent, Project, PaymentPlan, Document
import os
...
def insert_full_property_record(property_data):
    ...
    try:
        ...
        update_cols.update({'agency_id': agency_id, 'agent_id': agent_id, 'project_id': project_id})
        stmt = stmt.on_conflict_do_update(
        ...
        session.flush()
        prop = session.query(Property).filter_by(external_id=prop_fields['external_id']).first()
        if prop is None:
            raise ValueError(f"Property with external_id {prop_fields['external_id']} not found after insertion")
        property_id = prop.id
...
def backfill_properties_to_normalized_tables():
    ...
    try:
        ...
        for prop in properties:
                ...
                if not agency_data and extra_fields.get('agency_name'):
                    ...
                    if isinstance(agency_name_obj, dict):
                        agency_data = {
                            ...
                            'logo': agency_name_obj.get('logo', {}).get('url') if agency_name_obj.get('logo') else None
                        }
            ...
            locations = []
            if extra_fields is not None and isinstance(extra_fields, dict):
                locs = extra_fields.get('location_hierarchy') or extra_fields.get('location')
                if isinstance(locs, list):
                    for l in locs:
                        locations.append({
                            ...
                            'name': l.get('name'),
                            'name_l1': l.get('name_l1'),
                            'slug': l.get('slug'),
                            'slug_l1': l.get('slug_l1'),
                        })
            ...
            if extra_fields is not None and isinstance(extra_fields, dict):
                ...
                if isinstance(photo_url_obj, dict) and photo_url_obj.get('url'):
                    media_items.append({
                        ...
                        'title': photo_url_obj.get('title') or f"Cover Photo"
                    })
            ...
            payment_plans = []
            if extra_fields is not None and isinstance(extra_fields, dict):
                plans = extra_fields.get('paymentPlans') or extra_fields.get('payment_plans') or []
                for plan in plans:
        ...
        session.commit()
        print(f"Successfully backfilled normalized tables for {len(properties)} properties")
    except Exception as e:
        session.rollback()
